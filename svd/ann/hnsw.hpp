...
namespace ann {
    ...
    struct SVDQuantizer {
        ...
        void save(FILE *fp) const {
            pecos::file_util::fput_multiple<size_t>(&feat_dim, 1, fp);
            ...
            pecos::file_util::fput_multiple<size_t>(&sz, 1, fp);
            if (sz) {
                pecos::file_util::fput_multiple<float>(&basis[0], sz, fp);
            }
        }

        void load(FILE *fp) {
            pecos::file_util::fget_multiple<size_t>(&feat_dim, 1, fp);
            size_t sz = 0;
            ...
            basis.resize(sz);
            if (sz) {
                pecos::file_util::fget_multiple<float>(&basis[0], sz, fp);
            }
        }

        __attribute__((__target__("avx512f")))
        inline void setup_lut(float* query, float* lut_ptr) const {
            for (size_t i = 0; i < low_rank; i++) {
                lut_ptr[i] = do_dot_product_simd(query, &basis[i * feat_dim], feat_dim);
            }
        }
    ...
    template<class FeatVec_T>
    struct GraphSVD : GraphBase {
        typedef FeatVec_T feat_vec_t;
        SVDQuantizer quantizer;
        index_type num_node;
        ...
        void build_graph(GraphL0<feat_vec_t>& G) {
            ...
            size_t feat_dim = G.feat_dim;
            code_offset = neighbor_size;

            // compute SVD low-rank vectors for each node
            //int num_sample_node = static_cast<int>(num_node * 0.1);
            Eigen::MatrixXf X(num_node, feat_dim);
            for(size_t i = 0; i < num_node; i++){
                for(size_t j = 0; j < feat_dim; j++){
                    X(i,j) = G.get_node_feat(i).val[j];
                }
            }
            ...
            //Eigen::MatrixXf V = SVD.matrixV();            
            Eigen::MatrixXf RR = U * S;
            quantizer.basis.resize(feat_dim * low_rank);
            quantizer.feat_dim = feat_dim;
            for (size_t i = 0; i < low_rank; i++) {
                for (size_t j = 0; j < feat_dim; j++) {
                    quantizer.basis[i * feat_dim + j] = V(j,i);
                }
            }

            quantizer.buffer.resize(num_node * ( sizeof(index_type) + low_rank * sizeof(float)));
            for (size_t i = 0; i < num_node; i++) {
                memcpy(&quantizer.buffer[i * (sizeof(index_type) + sizeof(float) * low_rank)], &low_rank, sizeof(index_type));
                for(size_t j = 0; j < low_rank; j++) { 
                    memcpy(&quantizer.buffer[i * (sizeof(index_type) + sizeof(float) * low_rank) + sizeof(index_type) + j * sizeof(float)], &RR(i,j), sizeof(float));
                }
            }

            if ( save_per_edge ) {
                node_mem_size = neighbor_size + max_degree * (sizeof(index_type) + low_rank * sizeof(float));
            } else {
                node_mem_size = neighbor_size; 
            }
            ...
            for (size_t i = 0; i < num_node; i++) {
                if ( save_per_edge ) {
                    ...
                    for (index_type j = 0; j < size; j++) {
                        ...
                        memcpy(&buffer[mem_start_of_node[i] + (1 + G.max_degree) * sizeof(index_type) + j * (sizeof(index_type) + sizeof(float) * low_rank)], &quantizer.buffer[member * (sizeof(index_type) + sizeof(float) * low_rank)], sizeof(index_type) + sizeof(float) * low_rank);
                    }
                } else {
                    memcpy(&buffer[mem_start_of_node[i]], &G.buffer[G.mem_start_of_node[i]], (1 + G.max_degree) * sizeof(index_type));
                }
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    struct HNSWSVD {
        ...
        GraphL1 graph_l1;                       // neighborhood graphs from level 1 and above
        GraphSVD<feat_vec_t> graph_svd;   // Productquantized4Bits neighborhood graph built from graph_l0
        HNSWSVD() {
        ...
        struct Searcher : SetOfVistedNodes<unsigned short int> {
            typedef SetOfVistedNodes<unsigned short int> set_of_visited_nodes_t;
            typedef HNSWSVD<dist_t, FeatVec_T> hnswsvd_t;
            typedef heap_t<pair_t, std::less<pair_t>> max_heap_t;
            typedef heap_t<pair_t, std::greater<pair_t>> min_heap_t;

            const hnswsvd_t* hnsw;
            max_heap_t topk_queue;
            min_heap_t cand_queue;
            alignas(64) std::vector<uint8_t> lut;
            alignas(64) std::vector<float> query_lowrank;
            alignas(64) std::vector<char> query_vec;
            //feat_vec_t query_feat_vec;

            Searcher(const hnswsvd_t* _hnsw=nullptr):
                SetOfVistedNodes<unsigned short int>(_hnsw? _hnsw->num_node : 0),
                hnsw(_hnsw)
            {}
            ...
            max_heap_t& search_finger(const feat_vec_t& query, index_type init_node, index_type efS, index_type level) {
                if (save_per_edge) {
                    return hnsw->search_finger_per_edge(query, init_node, efS, level, *this);
                }
                else {
                    return hnsw->search_finger(query, init_node, efS, level, *this);
                }
            }

            void setup_lut(float* query) {
                query_lowrank.resize(low_rank);
                query_vec.resize(sizeof(index_type) + sizeof(float) * low_rank);
                memcpy(query_vec.data(), &low_rank, sizeof(index_type)); 
                hnsw->graph_svd.quantizer.setup_lut(query, query_lowrank.data());
                memcpy(query_vec.data() + sizeof(index_type), query_lowrank.data(), sizeof(float) * low_rank); 
                //query_feat_vec(query_vec);
            }

            void approximate_distance(size_t neighbor_size, const char* neighbor_codes) {
                // pass searcher to group_distance
                //hnsw->graph_svd.quantizer.approximate_neighbor_group_distance(neighbor_size, appx_dist.data(), neighbor_codes, lut.data(), scale, bias);
            }

            max_heap_t& search_level(const feat_vec_t& query, index_type init_node, index_type efS, index_type level) {
                if (save_per_edge) {
                    return hnsw->search_level_per_edge(query, init_node, efS, level, *this);
                } 
                else { 
                    return hnsw->search_level(query, init_node, efS, level, *this);
                }
        ...
        static nlohmann::json load_config(const std::string& filepath) {
            ...
            auto j_param = nlohmann::json::parse(json_str);
            std::string hnsw_t_cur = pecos::type_util::full_name<HNSWSVD>();
            std::string hnsw_t_inp = j_param["hnsw_t"];
            if (hnsw_t_cur != hnsw_t_inp) {
                throw std::invalid_argument("Inconsistent HNSW_T: hnsw_t_cur = " + hnsw_t_cur  + " hnsw_t_cur = " + hnsw_t_inp);
            }
        ...
        void save_config(const std::string& filepath) const {
            nlohmann::json j_params = {
                {"hnsw_t", pecos::type_util::full_name<HNSWSVD>()},
                {"version", "v1.0"},
                {"train_params", {
                    {"num_node", this->num_node},
        ...
        void train(
            ...
            graph_svd.build_graph(hnsw->graph_l0);
            delete hnsw;
        ...
        max_heap_t& predict_single(const feat_vec_t& query, index_type efS, index_type topk, Searcher& searcher, index_type num_rerank) const {
            ...
            if (scheme == "AppxOnly") {
                ...
                searcher.search_finger(query, curr_node, std::max(efS, topk), 0);
            }
            else if (scheme == "MIX") {
                searcher.search_mix(query, curr_node, std::max(efS, topk), 0);
            }
            else {
                throw std::runtime_error("Specify a valid search scehem");
            }
        ...
        max_heap_t& search_finger_per_edge(
            const feat_vec_t& query,
            index_type init_node,
            index_type efS,
            index_type level,
            ...
            searcher.mark_visited(init_node);
            int counter = 0; 
            while (counter < 5 and !cand_queue.empty()) {
                pair_t cand_pair = cand_queue.top();
                if (cand_pair.dist > topk_ub_dist) {
                    break;
                }
                ...
                if (neighbors.degree() != 0) {
                    ...
                    for (index_type j = 0; j <= max_j; j++) {
                        ...
                        if (!searcher.is_visited(next_node)) {
                            ...
                            next_lb_dist = feat_vec_t::distance(
                                query,
                                feature_vec.get_node_feat(next_node)
                            );
                            if (topk_queue.size() < efS || next_lb_dist < topk_ub_dist) {
                                cand_queue.emplace(next_lb_dist, next_node);
                                feature_vec.prefetch_node_feat(cand_queue.top().node_id);
                                topk_queue.emplace(next_lb_dist, next_node);
                                if (topk_queue.size() > efS) {
                                    topk_queue.pop();
                                }
            ...
            while (!cand_queue.empty()) {
                ...
                const auto neighbors = G0Q->get_neighborhood(cand_node, level);
                if (neighbors.degree() != 0) {
                    size_t mem_ptr = cand_node * G0Q->node_mem_size + (1 + G0Q->max_degree) * sizeof(index_type);
                    _mm_prefetch((char*) &G0Q->buffer[mem_ptr], _MM_HINT_T0);
                    index_type max_j = neighbors.degree() - 1;
                    for (index_type j = 0; j <= max_j; j++) {
                        _mm_prefetch((char*) &G0Q->buffer[mem_ptr + node_mem_size], _MM_HINT_T0);
                        appx_dist[j] = feat_vec_t::distance(
                            searcher.query_vec.data(),
                            feat_vec_t(const_cast<void*>( static_cast<const void*>((&graph_svd.buffer[mem_ptr]))))
                        );
                        mem_ptr += node_mem_size;
                        if (!searcher.is_visited(neighbors[j]) and appx_dist[j] < topk_ub_dist) { 
                            searcher.mark_visited(neighbors[j]);
                            feature_vec.prefetch_node_feat(neighbors[j]);
                        } 
                        else { 
                            appx_dist[j] = std::numeric_limits<float>::max();
                        }
                    }
                    for (index_type j = 0; j <= max_j; j++) {
                        if (appx_dist[j] < topk_ub_dist) {
                            auto next_node = neighbors[j];
                            ...
                            cand_queue.emplace(next_lb_dist, next_node);
                            _mm_prefetch((char*) &G0Q->buffer[cand_queue.top().node_id * G0Q->node_mem_size], _MM_HINT_T0);
        ...
        max_heap_t& search_finger(
            ...
            while (!cand_queue.empty()) {
                ...
                const auto neighbors = G0Q->get_neighborhood(cand_node, level);
                if (neighbors.degree() != 0) {
                    _mm_prefetch((char*) &G0Q->quantizer.buffer[neighbors[0] * node_mem_size], _MM_HINT_T0);
                    feature_vec.prefetch_node_feat(neighbors[0]);
                    ...
                    index_type max_j = neighbors.degree() - 1;
                    for (index_type j = 0; j <= max_j; j++) {
                        _mm_prefetch((char*) &graph_svd.quantizer.buffer[neighbors[std::min(j + 1, max_j)] * node_mem_size], _MM_HINT_T0);
                        feature_vec.prefetch_node_feat(neighbors[std::min(j + 1, max_j)]);
                        auto next_node = neighbors[j];
                        if (!searcher.is_visited(next_node)) {
                            searcher.mark_visited(next_node);
                            ...
                            next_lb_dist = feat_vec_t::distance(
                                searcher.query_vec.data(),
                                feat_vec_t(const_cast<void*>( static_cast<const void*>((&graph_svd.quantizer.buffer[next_node * node_mem_size]))))
                            );
                            if (topk_queue.size() < efS || next_lb_dist < topk_ub_dist) {
                                ...
                                cand_queue.emplace(next_lb_dist, next_node);
                                _mm_prefetch((char*) &G0Q->quantizer.buffer[cand_queue.top().node_id * low_rank], _MM_HINT_T0);
        ...
        max_heap_t& search_mix_per_edge(
            const feat_vec_t& query,
            index_type init_node,
            index_type efS,
            index_type level,
            ...
            while (!cand_queue.empty()) {
                ...
                if (neighbors.degree() != 0) {
                    ...
                    for (index_type j = 0; j <= max_j; j++) {
                        ...
                        if (!searcher.is_visited(next_node)) {
                            searcher.mark_visited(next_node);
                            dist_t next_lb_dist = appx_dist[j];
                            if (topk_queue.size() < efS || next_lb_dist < topk_ub_dist) {
                                cand_queue.emplace(next_lb_dist, next_node);
                                _mm_prefetch((char*) &G0Q->quantizer.buffer[cand_queue.top().node_id * low_rank], _MM_HINT_T0);
        ...
        max_heap_t& search_level_per_edge(
            const feat_vec_t& query,
            index_type init_node,
            index_type efS,
            index_type level,
