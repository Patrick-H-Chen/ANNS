...
void run_dense(std::string data_dir , char* model_path, index_type M, index_type efC, index_type max_level, int threads, int efs) {
    ...
    // model prepare
    index_type topk = 10;
    pecos::ann::HNSWSVD<float, feat_vec_t> indexer;
...
int main(int argc, char** argv) {
    ...
    low_rank = atoi(argv[9]);
    scheme = argv[10];
    int save_mode = atoi(argv[11]);
    if (save_mode == 1) {
        save_per_edge = true;
    } 
    else {save_per_edge = false;}
    
    index_type max_level = 8;
    char model_path[2048];
    sprintf(model_path, "%s/pecos.%s.M-%d_efC-%d_t-%d_rank-%d_scheme-%s_edge-%d.bin", model_dir.c_str(), space_name.c_str(), M, efC, threads, low_rank, scheme.c_str(), save_per_edge);
